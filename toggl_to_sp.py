...
def parse_toggl_datetime(date_str: str, time_str: str) -> datetime:
    return datetime.strptime(
        f"{date_str.strip()} {time_str.strip()}", "%Y-%m-%d %H:%M:%S"
    )
...
def make_project(pid, title, task_ids):
    return {
        ...
        "backlogTaskIds": [],
        "noteIds": [],
        "advancedCfg": _ADVANCED_CFG,
        "theme": _PROJECT_THEME,
        "taskIds": task_ids,
        "icon": None,
        "id": pid,
        "title": title,
...
def migrate(backup_path, csv_paths, output_path):
    ...
    for row in all_rows:
        ...
        description = (
            row.get("Description") or row.get("Task") or "Untitled"
        ).strip() or "Untitled"
        ...
        tags = (
            {t.strip() for t in tags_raw.split(",") if t.strip()} if tags_raw else set()
        )
    ...
    for key, entry in agg.items():
        ...
        new_archive_tasks.append(
            make_archive_task(
                ...
                project_id=pid,
                tag_ids=tag_ids,
                time_spent_on_day={date_str: dur_ms},
                time_spent=dur_ms,
                created_ms=created_ms,
                done_on_ms=end_ms,
            )
    ...
    for key, nt in new_tags.items():
        tid = nt["id"]
        d["tag"]["entities"][tid] = make_tag(
            tid, nt["title"], nt["task_ids"], nt["created_ms"]
        )
...
def main():
    ...
    parser.add_argument(
        "--backup", required=True, help="Path to existing SP backup JSON (used as base)"
    )
    parser.add_argument(
        "--csvs",
        nargs="+",
